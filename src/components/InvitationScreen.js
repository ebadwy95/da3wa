...
// The invitation screen, shared by a guest's own invite link
...
export function LoadingCard({ lang }) {
  return (
    <PageShell lang={lang}>
...
/**
 * The invitation, for a guest (guestId + token) or as a preview for the couple
...
export function InvitationScreen({ guestId, token, previewEventId, initialLang }) {
  ...
  // overrides it — that is how the dashboard previews each card, and how a
  ...
  const load = useCallback(
    (requested) => {
      const langQ = requested ? `lang=${requested}` : "";
      const url = preview
        ? `/api/events/${previewEventId}/preview?${langQ}`
        : `/api/guests/${id}?t=${encodeURIComponent(token || "")}${langQ ? `&${langQ}` : ""}`;
      return fetch(url)
        .then(async (res) => {
    ...
    [id, token, preview, previewEventId]
  );

  useEffect(() => {
    if (!id && !preview) return;
    load(lang);
  }, [id, preview, lang, load]);

  useEffect(() => {
    // A preview has no guest to authorise the wall with; it shows it empty.
    if (tab !== "wall" || !state.event || preview) return;
  ...
  return (
    <PageShell theme={event.inviteTheme} lang={language}>
      ...
      {preview && (
        <p className="chip chip-info" style={{ fontSize: "var(--text-xs)", padding: ".35rem .8rem" }}>
          {ui.previewBanner}
        </p>
      )}
        ...
        >
          ...
          {guest.status === "pending" && (
            <div className="flex flex-col gap-4">
              ...
              {previewTapped && <p className="hint" style={{ textAlign: "center" }}>{ui.previewAction}</p>}
            </div>
          )}
          ...
          <Reveal className="inv-sec pad" delay={60}>
            ...
            <div className="flex flex-col gap-2" style={{ textAlign: "start" }}>
              ...
              {previewTapped && <p className="hint">{ui.previewAction}</p>}
              <button
                onClick={sendWish}
